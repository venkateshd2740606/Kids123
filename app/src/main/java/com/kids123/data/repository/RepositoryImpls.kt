package com.kids123.data.repository
...
import com.kids123.data.mapper.DataMappers
...
import com.kids123.engine.PuzzleProfileEngine
import com.kids123.engine.Kids123Engine
import com.kids123.engine.Kids123Generator
import com.kids123.engine.TutorialLevels
import com.kids123.util.DateUtils
...
class GameRepositoryImpl @Inject constructor(
    ...
    override suspend fun createNewGame(difficulty: Difficulty, levelNumber: Int): Kids123Game {
        ...
        val seed = Kids123Generator.seedFromLevelNumber(levelNumber, difficulty)
        val level = Kids123Generator.generate(seed, levelNumber, difficulty, generationProfile)
        return Kids123Engine.createInitialGame(level)
    }
    ...
    override suspend fun createGameFromSeed(
        ...
        val level = Kids123Generator.generate(seed, levelNumber, difficulty)
        return Kids123Engine.createInitialGame(level)
    }

    override suspend fun createTutorialGame(tutorialIndex: Int): Kids123Game? {
        val level = TutorialLevels.getTutorialLevel(tutorialIndex) ?: return null
        return Kids123Engine.createInitialGame(level)
    }

    override suspend fun createEndlessGame(wave: Int): Kids123Game {
        val seed = wave.toLong() * 7919L + System.currentTimeMillis() % 1000
        val level = Kids123Generator.generate(seed, wave, Difficulty.ENDLESS)
        return Kids123Engine.createInitialGame(level)
    }

    override suspend fun saveGame(game: Kids123Game): Long {
    ...
    override suspend fun getGame(gameId: Long): Kids123Game? =
        gameDao.getById(gameId)?.let { DataMappers.fromEntity(it) }

    override suspend fun getInProgressGame(): Kids123Game? =
        gameDao.getInProgress()?.let { DataMappers.fromEntity(it) }

    override fun observeInProgressGame(): Flow<Kids123Game?> =
        gameDao.observeInProgress().map { it?.let { e -> DataMappers.fromEntity(e) } }

    override suspend fun completeGame(game: Kids123Game): Kids123Game {
    ...
    override suspend fun getLevel(seed: Long, levelNumber: Int, difficulty: Difficulty): Kids123Level =
        Kids123Generator.generate(seed, levelNumber, difficulty)
}
...
class ChallengeRepositoryImpl @Inject constructor(
    ...
    override suspend fun getChallengeGame(record: ChallengeRecord): Kids123Game {
        val levelNumber = record.key.hashCode().and(0x7FFFFFFF) % 5000 + 1
        val level = Kids123Generator.generateForChallenge(
...
class ProgressionRepositoryImpl @Inject constructor(
    ...
    override suspend fun updateStatsAfterGame(game: Kids123Game) {
    ...
    private suspend fun updatePuzzleProfile(game: Kids123Game) {
    ...
    private fun updateFastest(current: Long, game: Kids123Game, difficulty: Difficulty): Long {
...
class PreferencesRepositoryImpl @Inject constructor(
    private val dataStore: com.kids123.data.local.PreferencesDataStore
) : com.kids123.domain.repository.PreferencesRepository {

    override fun getUserPreferences() = dataStore.preferencesFlow

    override suspend fun updatePreferences(
        transform: (com.kids123.domain.model.UserPreferences) -> com.kids123.domain.model.UserPreferences
